...
import {ShortUrlData} from "@/components/link/link-form";
...
export default function CongratulationsModal({ isOpen, onCloseAction, shortUrlData }: CongratulationsModalProps) {
  ...
  const truncateText = (text: string, maxLength: number, mobileMaxLength?: number) => {
      ...
      if (typeof window !== "undefined") {
          effectiveMaxLength = window.innerWidth < 640 ? mobileMaxLength || Math.floor(maxLength * 0.7) : maxLength
      }
    ...
    return (
        <TooltipProvider>
            <Dialog open={isOpen} onOpenChange={onCloseAction}>
                <DialogContent className="w-[calc(100vw-2rem)] max-w-lg mx-auto p-4 sm:p-6 dark:border-gray-800">
                    ...
                    <div className="space-y-4 sm:space-y-5">
                        ...
                        {shortUrlData && (
                            <div className="space-y-3 sm:space-y-4">
                                        ...
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <p className="text-xs text-gray-500 dark:text-gray-400 cursor-help truncate">
                                                    {truncateText(shortUrlData.original_url, 50, 25)}
                                                </p>
                                            </TooltipTrigger>
                                ...
                                {/* Short URL Display */}
                                <div className="p-2 sm:p-3 bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-950/30 dark:to-blue-950/30 rounded-lg border border-indigo-100 dark:border-indigo-800">
                                            ...
                                            <Tooltip>
                                                <TooltipTrigger asChild>
                                                    <code className="text-xs sm:text-sm font-mono text-indigo-700 dark:text-indigo-300 cursor-help block truncate">
                                                        {truncateText(shortUrlData.short_url, 50, 20)}
                                                    </code>
                                                </TooltipTrigger>
                                                <TooltipContent side="bottom" className="max-w-xs sm:max-w-sm">
